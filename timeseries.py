...
def main():
    ...
    if len(args.input) != len(args.names):
        raise IOError(
            "Input error. Please provide the same number of model "
    ...
    if df.empty:
        raise ValueError(
            "Empty dataframe. Check location ID. Options are : "
            + "', '".join(locations)
        )
    ...
    lg = fig.legend(
        by_label.values(), by_label.keys(), ncol=len(args.names) + 2, loc="upper center"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="subset",
        required=False,
        help="Data subset (train, test, val).",
        default="all",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default="all",
        help="Start date. " "Use a format that pandas understands",
    )
